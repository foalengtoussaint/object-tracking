...
# fusion params — drink gate follows van Andel (PMC5933268): the drinking phase
# is "face-to-glass distance below 15% of steady state". We read steady-state as
# the resting (rest_pre) source→mouth distance and place the near-mouth membership
# midpoint there, so "near" is self-normalised per source AND per trial.
DRINK_FRAC     = 0.15    # van Andel: drink when dist < 15% of steady-state distance
...
def evidence(dist_to_mouth, speed, conf):
    ...
    Distance-to-mouth IS the drink signal. Following van Andel (PMC5933268) the
    drink trigger is "distance below 15% of steady state": steady_state = the
    resting source->mouth distance (robust high percentile, the cup-on-table /
    arm-down baseline). The near-mouth membership crosses 0.5 at DRINK_FRAC *
    steady_state, so the gate is self-normalised per source and per trial.

    SLOW is only a GATE: a fast-moving frame cannot be a dwell, so it caps the
    evidence, but a slow frame does not dilute a clear near-mouth reading.
    ...
    dmin = np.nanpercentile(d, 5)                  # closest approach to the mouth
    # van Andel triggers drink when distance falls below 15% of steady state. Their
    # face marker is on the FOREHEAD (an offset reference, not a lip-contact point),
    # so even there the glass approaches but never reaches it; our head67 is a head
    # CENTROID, deeper still. Neither hits zero, so a literal 15%-of-absolute is
    # unreachable here. We apply the 15% as a fraction of the rest->closest
    # EXCURSION instead (drink = 85% of the way in from rest), which fires on the
    # same physical 'glass at the mouth' event regardless of how far inside the
    # head the reference point sits.
...
def fuse(trial):
    ...
    # 6 Hz zero-phase Butterworth on each source path (van Andel filter), then
    # derive distance-to-mouth and speed from the filtered positions
    cup_i = sc._butter_lp(cup_i, FPS)
    wrist_i = sc._butter_lp(wrist_i, FPS)
    head_i = sc._butter_lp(head_i, FPS)
