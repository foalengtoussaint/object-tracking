...
# Rigid head cluster (present on 737/772 cleaned trials, fully gap-filled). FHD is the
# forehead apex; the four *_[FB]Hd markers box the head. The study has NO mouth marker,
# so the mouth is derived as an offset from the front-head pair (see mouth_proxy()).
...
class CupTrial:
    ...
    def centroid(self, despike: bool = True, max_step_mm: float = 50.0) -> np.ndarray:
        ...
        NOTE: average ONLY the cup markers. The cleaned C3Ds also carry 5 HEAD markers;
        averaging all markers would pull the 'cup' centroid ~350 mm toward the head and
        silently corrupt every downstream use (sync/Kabsch, gt_quality, cup_to_mouth)."""
        idx = {l: i for i, l in enumerate(self.labels)}
        cup_cols = [idx[m] for m in CUP_MARKERS if m in idx]
        cup = self.markers[:, cup_cols] if cup_cols else self.markers
    ...
    def head_frame(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Per-frame head-local axes + front-head centre, from the rigid head cluster.

        Returns (front_centre, fwd, right, down) each (T,3): fwd points from the back-head
        pair to the front-head pair (face direction), right from L_FHd to R_FHd, down is
        their cross product (into the body). NaN where any of the 4 corner markers is
        missing. FHD (forehead apex) is not required — the 4 corner markers define the
        rigid frame. This is the anatomical frame the mouth offset is expressed in."""
        idx = {l: i for i, l in enumerate(self.labels)}
        m = self.markers
        lf, rf = m[:, idx["L_FHd"]], m[:, idx["R_FHd"]]
        lb, rb = m[:, idx["L_BHd"]], m[:, idx["R_BHd"]]
        front_c = (lf + rf) / 2.0
        fwd = _normalize(front_c - (lb + rb) / 2.0)
        right = _normalize(rf - lf)
        down = _normalize(np.cross(fwd, right))
        return front_c, fwd, right, down

    def mouth_proxy(self, down_mm: float = 70.0, fwd_mm: float = 30.0,
                    calibrate_apex: bool = True, rim_mm: float = 50.0) -> np.ndarray:
        """(T,3) mm mouth point in the lab frame, offset from the front-head centre.

        There is no mouth marker, so the mouth is placed below (down_mm) and slightly in
        front of (fwd_mm) the front-head centre in the rigid head frame. With
        calibrate_apex=True the DOWN offset is adjusted per trial so the cup-to-mouth
        MINIMUM lands at `rim_mm` (cup rim ~5 cm from the lips at the drinking apex) —
        self-consistent per participant/head size. This uses the cup to set the mouth
        depth, a mild circularity, but the dwell interval keys off the distance RANGE
        (van Andel 15%-of-steady-state), which is insensitive to the absolute floor.
        Returns NaN where the head cluster is missing."""
        front_c, fwd, right, down = self.head_frame()
        mouth = front_c + fwd * fwd_mm + down * down_mm
        if calibrate_apex:
            cup = self.centroid()
            d = np.linalg.norm(cup - mouth, axis=1)
            if np.isfinite(d).any():
                # shift the mouth straight along `down` so min(cup-mouth) == rim_mm.
                # first-order: moving the mouth toward the cup at apex reduces the min
                # ~1:1, so add the residual (min - rim) of extra down-offset.
                shift = float(np.nanmin(d)) - rim_mm
                mouth = mouth + down * shift
        return mouth

    def cup_to_mouth(self, **kw) -> np.ndarray:
        """(T,) mm distance from the cup centroid to the mouth proxy (NaN-safe)."""
        return np.linalg.norm(self.centroid() - self.mouth_proxy(**kw), axis=1)

    def head_marker_pts(self) -> np.ndarray:
        """(T,5,3) mm lab-frame positions of the 5 head markers in HEAD_MARKERS order
        (FHD, L_FHd, R_FHd, L_BHd, R_BHd). NaN for any marker not present/missing. Raw
        points — the caller anchors them (e.g. to the cup rest origin + rep basis) so cup
        and head live in one shared, person-comparable space with no mouth assumption."""
        idx = {l: i for i, l in enumerate(self.labels)}
        out = np.full((self.n_frames, len(HEAD_MARKERS), 3), np.nan)
        for k, name in enumerate(HEAD_MARKERS):
            if name in idx:
                out[:, k] = self.markers[:, idx[name]]
        return out

    def head_centroid(self) -> np.ndarray:
        """(T,3) mm mean of the head markers — a single stable head point. This is the
        landmark the FUTURE biomechanical model will provide (one head point, not a
        cluster), so the dwell truth keys off cup->this, no mouth proxy / head frame."""
        idx = {l: i for i, l in enumerate(self.labels)}
    ...
    def cup_to_head(self) -> np.ndarray:
        """(T,) mm cup-centroid to head-centroid distance (the drink signal, no mouth proxy)."""
        return np.linalg.norm(self.centroid() - self.head_centroid(), axis=1)

    def cup_in_head_frame(self) -> np.ndarray:
        """(T,3) mm cup position in the HEAD's own frame — the raw geometric relation the
        model can learn from with NO mouth assumption. Coordinates are the cup relative to
        the front-head centre, projected onto (right, fwd, down): [+right of face, +in
        front of face, +below face]. Tilt-invariant (rotates with the head) and person-
        invariant (origin at the head, not the lab). NaN where the head cluster is missing.
        'Cup at the mouth' becomes a learnable region in this frame instead of a hand-set
        offset. Also drop-in for a future VIDEO head-pose proxy (same frame definition)."""
        front_c, fwd, right, down = self.head_frame()
        rel = self.centroid() - front_c                         # (T,3) lab-frame offset
        return np.stack([(rel * right).sum(1),                  # right of face
                         (rel * fwd).sum(1),                    # in front of face
                         (rel * down).sum(1)], 1)               # below face
