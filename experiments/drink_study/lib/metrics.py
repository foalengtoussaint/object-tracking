...
def train_with_metrics(data: Path, weights: str, project: str, name: str,
                       ...
                       agr_hand: str = "right", config: dict | None = None,
                       earlystop_on_f1: bool = False, f1_patience: int = 4,
                       f1_delta: float = 0.005, f1_min_epoch: int = 6,
                       save_every_epoch: bool = False) -> tuple[Path, dict]:
    """Early-stop modes (mutually exclusive):
      - long_run=True            : no early-stop, save every `save_period` epoch.
      - earlystop_on_f1=True     : stop on HELD-OUT F1 plateau (a presence metric
                                   that saturates ~ep8-10; val-loss keeps dropping =
                                   overfit). Set save_every_epoch=True to ALSO keep
                                   every epoch checkpoint so the per-epoch analysis
                                   (eval_by_epoch.json) still works.
      - default (else)           : val-loss plateau early-stop.
    """
    from ultralytics import YOLO
    model = YOLO(weights)

    if earlystop_on_f1:                    # held-out F1 plateau early-stop
        st = {"best": -1.0, "best_ep": 0}

        def cb(tr):
            ep = int(getattr(tr, "epoch", 0)) + 1
            last = Path(getattr(tr, "save_dir", ".")) / "weights" / "last.pt"
            if not last.exists():
                return
            r = eval_gate(str(last), Path(holdout_clips), eval_conf)
            f1 = _f1(r.metrics["overall_recall"], r.metrics["overall_p_loose"])
            if f1 > st["best"] + f1_delta:
                st["best"], st["best_ep"] = f1, ep
            print(f"[f1-earlystop] ep{ep} held-out F1={f1:.3f} "
                  f"(best {st['best']:.3f}@{st['best_ep']})", flush=True)
            if ep >= f1_min_epoch and (ep - st["best_ep"]) >= f1_patience:
                print(f"[f1-earlystop] held-out F1 plateaued -> stop at ep{ep}", flush=True)
                tr.stop = True

        model.add_callback("on_fit_epoch_end", cb)
    elif not long_run:                     # val-loss plateau early-stop
    ...
    # keep every epoch when long_run OR explicitly requested (for per-epoch analysis)
    sp = save_period if (long_run or save_every_epoch) else -1
    model.train(data=str(data), epochs=max_epochs, imgsz=imgsz, batch=batch,
                project=project, name=name, exist_ok=True, save_period=sp)
